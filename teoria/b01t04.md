...
[delapuente/master-python-2019](https://github.com/delapuente/master-python-2019).
...
porciones entre ángulos `<` y `>` por lo que corresponda):
    ...
    ```bash
    $ git clone https://github.com/<tu-usuario>/master-python-2019.git
    ```

7. El comando anterior crea una carpeta `master-python-2019` donde se 
encuentra la copia del proyecto. Entra en esta carpeta:

    ```bash
    $ cd master-python-2019
    ```
...
La última orden, envía los cambios al histórico (también llamado "la historia
del repositorio").
...
[panel de _issues_](https://github.com/delapuente/master-python-2019/issues) 
...
[mejor traducción](https://github.com/delapuente/master-python-2019/issues/new?template=mejora-del-texto.md&title=El+texto+sobre+tal+cosa+podría+ser+más+sencillo)). Estas consisten en 
...
automáticamente a `delapuente/master-python-2019`, rama `master`. Utiliza los
...
(no es obligatorio pero se trata de un convenio):
    ...
    ```bash
    $ git remote add upstream https://github.com/delapuente/master-python-2019.git
    ```
