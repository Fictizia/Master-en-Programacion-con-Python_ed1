...
Resulta que PyCharm ya incluye un razonador de tipos que escanea tu código
conforme escribes. Sin embargo, nosotros vamos a usar mypy así que, para esta
...
dos veces porque mypy se quejará de que lo estás haciendo. Por tanto, en vez
de copiar y pegar los ejemplos, modificalos convenientemente o borra la vieja
definición antes de copiar la nueva.
...
`typing_extensions.Literal`. Por ejemplo:
    ...
    def get_rgb_value(colorname: Literal['red', 'green', 'blue']):
        ...
    get_rgb_value('pink')  # mypy indicates it is not a correct value.
    ```
...
2. Por cierto, de querer usar una tupla con nombre (`namedtuple`), podríamos
...
similar a `NamedTuple` permite dotar de cierta estructura a los diccionarios:
    ...
    ziltoid_id: Identification = {'name': 'Ziltoid'}  # mypy complains
    ```
    ...
    ```python
    class Identification(TypedDicts, total=False):
        name: str
        id: int
...
6. Las funciones tienen tipo `Callable`:
    ...
    def neg(a: int, b: int) -> int:
        return -a

    apply_binary(add, 5, 10)
    apply_binary(sub, 5, 10)
    apply_binary(neg, 5, 10)
    ```
...
9.  Realmente, no hace falta que restringir las operaciones binarias sobre
...
algo como:
    ...
    ```python
    StrOrBytes = TypeVar('A', str, bytes)
...
2. Resulta algo engorroso de escribir. Por ejemplo en la función:
    ...
    ```python
    def add(
        ma: Sequence[Sequence[complex]] = [[1, 0], [0, 1]],
        mb: Sequence[Sequence[complex]] = [[1, 0], [0, 1]]
        ) -> Sequence[Sequence[complex]] = [[1, 0], [0, 1]]:
        return ma + mb
    ```
...
poder expresar "la ausencia de un valor".
    ...
    def is_perfect_square(n: complex) -> bool:
        int_root = int(n ** 0.5)
        return int_root ** 2 == n
...
da `Optional[T]` para expresar lo mismo:
    ...
    if ...:
        nullable_int = find_if(is_perfect_square, [6, 7, 8, 9])
    ```
...
5. Con mypy podemos anotar los miembros de la instancia, tanto en el cuerpo
...
`typing_extensions.Final`:
    ...
    p = Point2D()
    Point2D.version = '5.0.0'  # mypy complains
    ```
...
8. mypy también puede representar el tipo de una clase con `typing.Type`:
    ...
    new_profile(Profile)
    new_profile(RichProfile)
    new_profile(object)  # mypy complains
    ```
...
1. En general, para cualquier razonador moderno, **las colecciones inmutables
son covariantes**:
...
4. Sin embargo, los invocables son **contravariantes** con sus argumentos:
    ...
    el `Callable[[Employee], float]` la que es subclase de
