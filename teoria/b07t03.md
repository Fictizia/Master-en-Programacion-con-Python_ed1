...
Como ves, `PubSubMessage` es una clase orientada a datos que, sencillamente,
...
La corrutina `publish()` toma una cola donde publicar los mensajes y un número
...
finalización del servicio. La excepción se produce en `run_forever()` y nunca
se ejecutan las dos últimas instrucciones.
...
`await` para obtener una concurrencia efectiva. Si esperamos par el publicador
a que termina y luego por el consumidor ¿qué diferencia hay con la versión
síncrona?
...
`asyncio.create_task()`, `asyncio.gather()` y `await`.
...
Gestionar señales es una capacidad del bucle de eventos. Escribe primero el
...
Fíjate en cómo el propio manejador añade una nueva tarea al bucle que se
