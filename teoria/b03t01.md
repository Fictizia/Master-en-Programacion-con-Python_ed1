...
## _Truth values_, y operaciones booleanas
...
El objeto `None` pertenece al tipo `NoneType`. Se suele utilizar `None` cuando
se desea indicar la ausencia de un valor. Como Python garantiza que sólo hay
un valor `None`, es seguro y más semántico comprobar este valor con `is`:
...
El objeto `NotImplemented` pertenece al tipo `NotImplementedType`. Se utiliza
en la definición de métodos de clase para indicar que el método existe pero
no está implementado.

### _Ellipsis_

El objeto `Ellipsis` o su forma alternativa `...` es del tipo `ellipsis`.
